...
Mirage is now running when our app boots up in development!  As the application attempts to fetch data from the network, Mirage will step in to handle the request.  The following `App` component fetches the two users that were defined in our `routes()` config from earlier.
